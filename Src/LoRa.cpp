...
LoRa::LoRa(SPI_HandleTypeDef* hspi, GPIO_TypeDef* gpio0, uint16_t dio0, GPIO_TypeDef* gpio1, uint16_t dio1,
		   ...
		   GPIO_TypeDef* gpiotx, uint16_t txenpin, GPIO_TypeDef* gpiocs, uint16_t cspin, GPIO_TypeDef* gpiorst, uint16_t rstpin) : SPI_Device(hspi)
{
	//ctor
	_gpio0 = gpio0;
	_dio0 = dio0;
...
int LoRa::send(uint16_t len, const uint8_t* data)
{
	...
	_queueCommand(WriteReg | RegFifoAddrPtr, 0x00);
